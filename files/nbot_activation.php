...
<style>
.field {
  width:300px;padding:10px;border:1px solid silver;color:#555;outline:none
}
.field2 {
  width:300px;padding:5px;border:1px solid silver;color:#555;outline:none
}
</style>
  ...
  <?php
              if($_POST){
                ...
                if(strpos($room,'https://plug.dj/') !== false){
                  ...
                  if($row['id']){
                    ...
                    $success = "1";
                    $error = "Community $room has been successfully activated!";
                  }
                } else {
                  $error = "Invalid room";
                }
        ...
        echo "<div style=\"width:100%;height:100px;background-color:$errClr;text-align:center;font-weight:bold;color:white;line-height:100px;\">$error</div>";
  ...
  <form method="POST">
    ...
    <tr>
      ...
      <th>Optionally data - you can edit this anytime</th>
    </tr>
    <tr>
      <td valign="top">
      ...
      <td valign="top">
            <p>BOT Name</p> <input  type="text" name="name" class="field2" value="NarcisBOT">
            <p>Language</p> <select class="field2" name="language">
            <?php while($langsKey = mysqli_fetch_object($langsQuery)){
              $langName = $langsKey->langName;
              if($langName == "English"){ $q = "selected=\"selected\""; } else { $q = ""; }
              echo "<option value=\"".$langsKey->lang."\" $q>".$langName."</option>";
            }
            ?></select>

            <p>Max !voteskip votes</p> <input  type="text" name="voteskipCount" class="field2" value="10">
            <p>Limit per song (seconds)</p> <input  type="text" name="songLimit" class="field2" value="420">
            <p>Roulette limit (seconds)</p> <input  type="text" name="rouletteTime" class="field2" value="30">
            <p>Song history limit (songs)</p> <input  type="text" name="historyLimit" class="field2" value="50">
            ...
            <select name="stats" class="field2">
              ...
              <p>Command trigger</p> <input  type="text" name="cmdTrigger" class="field2" value="!" maxlength="1">
              <p>Facebook page</p> <input  type="text" name="fbPage" class="field2" placeholder="http://">
              <p>Rules page (or text)</p> <input  type="text" name="rules" class="field2">
              <p>Limit for !dc (hours)</p> <input  type="text" name="dcLimit" class="field2" value="48">
              <p>Welcome message (@ = @mention)</p> <input  type="text" name="welcomeMessage" class="field2" value="Hello @ how are you?">
        </td>
      </tr>
      <tr>
        <td colspan="2">
